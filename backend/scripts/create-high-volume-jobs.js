...
const tariff_1 = require("../src/models/tariff");
const manager_1 = require("../src/models/manager");
...
class HistoricalJobsGenerator {
    ...
    async verifyUser() {
        ...
        this.user = await user_1.UserModel.findOne({ email: USER_EMAIL });
        if (!this.user) {
    ...
    async createManagers() {
        ...
        for (const data of managerData) {
            let manager = await manager_1.ManagerModel.findOne({ email: data.email });
            if (!manager) {
                const ManagerConstructor = mongoose_1.default.model('Manager');
    ...
    async createClients() {
        ...
        for (let i = 0; i < CLIENT_NAMES.length; i++) {
            ...
            while (attempt < 3 && !client) {
                try {
                    client = await client_1.ClientModel.findOne({
                        managerId: manager._id,
                        normalizedName: normalizedName
                    });
                    if (!client) {
                        const ClientConstructor = mongoose_1.default.model('Client');
    ...
    async createRoles() {
        ...
        for (const manager of this.managers) {
            for (const role of Object.keys(ROLE_RATES)) {
                let roleDoc = await role_1.RoleModel.findOne({
                    managerId: manager._id,
                    normalizedName: role.toLowerCase()
                });
                if (!roleDoc) {
                    const RoleConstructor = mongoose_1.default.model('Role');
                    try {
                        ...
                        if (error.code === 11000) {
                            ...
                            roleDoc = await role_1.RoleModel.findOne({
                                managerId: manager._id,
                                normalizedName: role.toLowerCase()
                            });
                            if (!roleDoc) {
    ...
    async createTariffs() {
        ...
        for (const client of this.clients) {
            ...
            for (const role of Object.keys(ROLE_RATES)) {
                ...
                const roleData = this.roles.find(r => r.managerId.equals(manager._id) && r.name === role);
                if (!roleData) {
                    console.warn(`  ⚠️  Role ${role} not found for manager ${manager.name}`);
                    continue;
                }
                // Add some rate variation (±$3)
                const baseRate = ROLE_RATES[role];
                const variation = Math.floor(Math.random() * 7) - 3;
                const finalRate = Math.max(15, baseRate + variation);
                let tariff = await tariff_1.TariffModel.findOne({
                    managerId: manager._id,
                    clientId: client._id,
                    roleId: roleData.roleId
                });
                if (!tariff) {
                    const TariffConstructor = mongoose_1.default.model('Tariff');
                    try {
                        ...
                        if (error.code === 11000) {
                            ...
                            tariff = await tariff_1.TariffModel.findOne({
                                managerId: manager._id,
                                clientId: client._id,
                                roleId: roleData.roleId
                            });
    ...
    generateMonthlyEvents(year, month) {
        ...
        for (let week = 0; week < 4; week++) {
            ...
            if (Math.random() > 0.15) {
                ...
                if (sundayEvent)
                    monthlyEvents.push(sundayEvent);
            }
    ...
    generateSingleEvent(year, month, day, timeOfDay) {
        ...
        const tariff = this.tariffs.find(t => t.managerId.equals(manager._id) &&
    ...
    async saveEvents() {
        ...
        for (let i = 0; i < this.eventsGenerated.length; i += batchSize) {
            ...
            await event_1.EventModel.insertMany(batch);
    ...
    calculateTotalEarnings() {
        return this.eventsGenerated.reduce((total, event) => {
            const tariff = this.tariffs.find(t => t.managerId.equals(event.managerId) &&
                t.clientId.equals(event.clientId) &&
                t.roleName === event.accepted_staff[0].role);
            return total + (event.approvedHours * (tariff?.rate || ROLE_RATES[event.accepted_staff[0].role]));
        }, 0);
    }
    calculateTotalHours() {
    ...
    async generateReport() {
        ...
        for (const event of this.eventsGenerated) {
            ...
            const tariff = this.tariffs.find(t => t.managerId.equals(event.managerId) &&
                t.clientId.equals(event.clientId) &&
                t.roleName === event.accepted_staff[0].role);
            month.earnings += event.approvedHours * (tariff?.rate || ROLE_RATES[event.accepted_staff[0].role]);
        }
...
//# sourceMappingURL=create-high-volume-jobs.js.map