...
void main() {
  ...
  group('Manager App Store Screenshots — Spanish', () {
    testWidgets('Capture all screens (ES)', (WidgetTester tester) async {
      ...
      final cardFinderEs = find.byType(Card);
      if (cardFinderEs.evaluate().isNotEmpty) {
        await tester.tap(cardFinderEs.first);
        await tester.pumpAndSettle();
  ...
  group('Subscription Paywall Screenshot', () {
    testWidgets('Capture paywall with all tiers', (WidgetTester tester) async {
      await registerMockDependencies();
      ...
      await tester.pumpWidget(
        MaterialApp(
          ...
          home: const SubscriptionPaywallPage(),
        ),
      );
      await tester.pumpAndSettle();

      // Screenshot Lite card (page 0)
      final pageView = find.byType(PageView);
      await tester.drag(pageView, const Offset(600, 0)); // swipe right to first
      await tester.drag(pageView, const Offset(600, 0));
      await tester.pumpAndSettle();
      await takeScreenshot('08_paywall_lite');

      // Screenshot Starter card (swipe left once)
      await tester.drag(pageView, const Offset(-300, 0));
      await tester.pumpAndSettle();
      await takeScreenshot('09_paywall_starter');

      // Screenshot Pro card (swipe left once more)
      await tester.drag(pageView, const Offset(-300, 0));
      await tester.pumpAndSettle();
      await takeScreenshot('10_paywall_pro');

      // Screenshot Business card
      await tester.drag(pageView, const Offset(-300, 0));
      await tester.pumpAndSettle();
      await takeScreenshot('11_paywall_business');
    });
  });
}
